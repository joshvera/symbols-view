...
export default class SymbolsView {
  ...
  async didConfirmSelection(tag) {
    const relativeToProject = fs.isFileSync(path.join(tag.directory, tag.file))
    const absolutePath = fs.isFileSync(tag.file)

    if (tag.file && !relativeToProject && !absolutePath) {
      ...
      await this.cancel();
      this.openTag(tag, relativeToProject);
    }
  }
  ...
  openTag(tag, isRelative) {
    ...
    if (tag.file) {
      const filepath = isRelative ? path.join(tag.directory, tag.file) : tag.file
